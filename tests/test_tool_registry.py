...
These tests introspect the registered tools via ``mcp.list_tools()`` and verify:
  ...
  6. Destructive tools (stop/reset/delete/rollback) have destructiveHint = True
"""
...
async def test_all_tools_have_hint_set():
    for tool in await _tools():
        ...
        assert has_hint, f"Tool '{tool.name}' has no readOnly/destructive hint"
...
async def test_read_tools_are_readonly():
    ...
    for name in READ_TOOL_NAMES:
        assert (
            tool_map[name].annotations.readOnlyHint is True
